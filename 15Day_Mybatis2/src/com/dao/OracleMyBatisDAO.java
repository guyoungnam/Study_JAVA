...
public class OracleMyBatisDAO {
	...
	public void multiInsert(SqlSession session, List<Dept> deptList) {
		int n = session.delete("com.dept.DeptMapper4.multiInsert", deptList);
		
	}

	public List<Dept> selectTopN(SqlSession session,int offset, int limit) {
		List<Dept> list = session.selectList("com.dept.DeptMapper5.selectTopN", null, new RowBounds(offset, limit));
		return list;
	}


	public void multiUpdateSeq(SqlSession session, List<String> deptnoes2) {
		int n = session.update("com.dept.DeptMapper6.multiUpdateSeq", deptnoes2);
